...
export interface ClusteredYaMapProps<T = any> extends ViewProps {
  ...
  renderMarker: (
    info: { point: Point; data: ListRenderItemInfo<T> },
    index: number
  ) => React.ReactElement;
  clusterColor?: string;
...
const YaMapNativeComponent = requireNativeComponent<
  Omit<ClusteredYaMapProps, "clusteredMarkers"> & { clusteredMarkers: Point[] }
>("ClusteredYamapView");
...
export class ClusteredYamap extends React.Component<ClusteredYaMapProps> {
  ...
  private getCommand(cmd: string): any {
    return Platform.OS === "ios"
      ? UIManager.getViewManagerConfig("ClusteredYamapView").Commands[cmd]
      : cmd;
  }
  ...
  private getProps() {
    ...
    processColorProps(
      props,
      "userLocationAccuracyFillColor" as keyof ClusteredYaMapProps
    );
    processColorProps(
      props,
      "userLocationAccuracyStrokeColor" as keyof ClusteredYaMapProps
    );
  ...
  render() {
    return <YaMapNativeComponent {...this.getProps()} ref={this.map} />;
  }
}
